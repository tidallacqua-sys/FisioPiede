...
function BaropodometriaIA({ clinicaId, planoIA, pacientes, setPacientes }) {
  ...
  async function analisar() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error((data && data.error && data.error.message) ? data.error.message : ("Erro " + res.status));
      const txt = (data.content || []).map(b => b.text || "").join("").trim();
      const m = txt.match(/\{[\s\S]*\}/);
      setResult(JSON.parse(m ? m[0] : txt));
    } catch (e) {
      const msg = (e && e.message) ? e.message : "erro desconhecido";
      setErro("Não foi possível analisar: " + msg + ". Se o arquivo for grande, tente um PDF menor ou um print do mapa de pressão. (A análise só funciona no site publicado.)");
    }
    setLoading(false);
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {result && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Btn v={salvoBaro ? "ghost" : "primary"} full sz="sm" disabled={salvoBaro} onClick={salvarHistorico}>{salvoBaro ? "✓ Salvo no histórico do paciente" : "💾 Salvar no histórico do paciente"}</Btn>
        </div>
      )}
...
const PLANOS_INFO = [
  ...
  { nome:"Enterprise", cor:"#F59E0B", icon:"👑", preco:2998, precoLabel:"R$ 2.998,00", precoParcela:"ou 12x de R$ 249,90", iaUsos:60, recursos:["Tudo do Premium","IA Clínica completa","Academy","Marketing Hub","60 análises de IA/mês"] },
];
