  ...
  const comprarIA50 = async () => {
    ...
    try {
      ...
      const r = await fetch("/api/stripe", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          valorCentavos: 100, descricao: "Pacote +50 análises de IA — FisioPiede (TESTE)", // TEMPORÁRIO R$1,00 — voltar para 4990
      ...
      {planoIA && planoIA !== "admin" && (() => {
        ...
        return (
          <div style={{ marginBottom:14 }}>
            ...
            {/* TESTE: botão sempre visível (trocar rest===0 ao finalizar o teste) */}
            {(rest === 0 || true) && (
              <div style={{ marginTop:8, padding:"12px 14px", background:`${C.accent}0A`, border:`1px solid ${C.accent}30`, borderRadius:9, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
                ...
                <Btn v="primary" sz="sm" disabled={comprandoIA} onClick={comprarIA50}>{comprandoIA?"Abrindo...":"✦ Comprar +50 análises (R$ 1,00 TESTE)"}</Btn>
              </div>
            )}
          </div>
        );
