...
function NovoPedidoModal({onClose,onSave,pacientes,clinicaName,clinicaId}) {
  const [f,setF] = useState({paciente:"",tipoPalmilha:"Inteira",tipoCalcado:"Tênis",flexibilidade:"Normal",cobertura:"EVA Perfurado",cor:"Preto",espessura:"4mm",comprimento:"",larguraAntePe:"",larguraCalcaneo:"",obsDireito:"",obsEsquerdo:"",obs:"",pagamento:"pix_agora",frete:"sedex"});
  ...
  const valorPalmilha = (PRECO_PALMILHA[f.pagamento]||PRECO_PALMILHA["pix_agora"]).valor;
  const valorFrete    = (FRETE_FP[f.frete]||FRETE_FP["sedex"]).valor;
  const valorTotal    = valorPalmilha + valorFrete;
  const pagarDepois   = f.pagamento === "pix_depois" || f.pagamento === "cartao_depois";
  ...
  const save = () => {
    ...
    onSave({id,clinicaId:clinicaId||null,clinica:clinicaName||"—",paciente:f.paciente,tipo:f.tipoPalmilha,status:"Recebido",data:new Date().toISOString().split("T")[0],rastreio:"",updatedAt:t,log:[`${t} — Recebido`],peso:pac.peso||"",altura:pac.altura||"",numeracao:pac.numeracao||"",arquivos,valorPalmilha,valorFrete,valorTotal,pagamentoLabel:(PRECO_PALMILHA[f.pagamento]||{}).label||"",freteLabel:(FRETE_FP[f.frete]||{}).label||"",pagarDepois,pago:false,...f});
    onClose();
  };
      ...
      <Card hover={false} p={0} style={{width:"100%",maxWidth:720,maxHeight:"93vh",overflowY:"auto",animation:"fadeUp .25s ease"}}>
        ...
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:20}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {Object.entries(PRECO_PALMILHA).map(([k,o])=>(
                <div key={k} onClick={()=>set("pagamento",k)} style={{padding:"11px 13px",borderRadius:9,cursor:"pointer",border:`2px solid ${f.pagamento===k?C.accent:C.border}`,background:f.pagamento===k?`${C.accent}10`:"transparent",transition:"all .15s"}}>
                  <div style={{fontSize:12,fontWeight:700,color:f.pagamento===k?C.accent:C.text}}>{o.label}</div>
                  <div style={{fontSize:15,fontWeight:900,color:f.pagamento===k?C.accent:C.sub,marginTop:3}}>R$ {brl(o.valor)}</div>
                </div>
              ))}
            </div>
            {pagarDepois && <div style={{fontSize:11,color:C.amber,marginTop:8}}>⏳ Este pedido será cobrado no fechamento do mês.</div>}
          </div>

          {/* Frete */}
          <div>
            <div style={{fontSize:10,color:C.accent,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",marginBottom:8}}>▸ Frete (obrigatório)</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {Object.entries(FRETE_FP).map(([k,o])=>(
                <div key={k} onClick={()=>set("frete",k)} style={{padding:"11px 13px",borderRadius:9,cursor:"pointer",border:`2px solid ${f.frete===k?C.green:C.border}`,background:f.frete===k?`${C.green}10`:"transparent",transition:"all .15s"}}>
                  <div style={{fontSize:12,fontWeight:700,color:f.frete===k?C.green:C.text}}>{o.label}</div>
                  <div style={{fontSize:15,fontWeight:900,color:f.frete===k?C.green:C.sub,marginTop:3}}>R$ {brl(o.valor)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{padding:14,background:`${C.green}07`,border:`1px solid ${C.green}22`,borderRadius:10}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.sub,marginBottom:5}}><span>Palmilha</span><span>R$ {brl(valorPalmilha)}</span></div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:C.sub,marginBottom:8}}><span>Frete ({(FRETE_FP[f.frete]||{}).label})</span><span>R$ {brl(valorFrete)}</span></div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:`1px solid ${C.green}22`,paddingTop:8}}>
              <div style={{fontSize:13,fontWeight:800,color:C.text}}>Total do pedido</div>
              <div style={{fontSize:24,fontWeight:900,color:C.green}}>R$ {brl(valorTotal)}</div>
            </div>
          </div>
