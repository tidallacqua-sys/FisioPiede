        ...
        <Card hover={false} p={28} style={{animation:"fadeUp .6s .1s ease both",opacity:0}}>
          {mode==="login" && (
            <div>
              ...
              <button onClick={()=>setMode("encontre")} style={{background:`${C.green}0C`,border:`1px solid ${C.green}30`,borderRadius:9,width:"100%",marginTop:14,padding:"10px 12px",fontSize:12,fontWeight:700,color:C.green,cursor:"pointer"}}>📍 Sou paciente — encontrar uma clínica perto de mim</button>
          ...
          {mode==="encontre" && (()=>{
            // Só clínicas com licença em dia aparecem no diretório público
            const ativas = (clinicas||[]).filter(c=>{ try { const s=ASSINATURA.calcular(c).status; return s==="Ativa"||s==="Em atraso"; } catch(e){ return true; } });
