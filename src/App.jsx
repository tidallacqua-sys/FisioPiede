        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          {!ehIOS && (()=>{
            ...
            <Card p={20} style={{border:`1px solid ${C.purple}30`,background:`${C.purple}06`}}>
              ...
              {/* 👤 Paciente da consulta + 🎤 microfone — antes mesmo de gravar */}
              <div style={{marginTop:12,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                <div style={{flex:"1 1 240px",minWidth:200}}>
                  <select value={pacConsulta} onChange={e=>{setPacConsulta(e.target.value);setSalvoConsulta(false);}}>
                    <option value="">👤 Paciente desta consulta (para salvar no prontuário)...</option>
              ...
              {notaConsulta&&(
                <div style={{marginTop:16,display:"flex",flexDirection:"column",gap:12}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
                    ...
                    <div style={{fontSize:11.5,fontWeight:700,color:pacConsulta?C.green:C.amber}}>{pacConsulta?`👤 ${pacConsulta}`:"⚠️ Selecione o paciente lá em cima para salvar"}</div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    {SECOES.map(([k,rotulo])=>(
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    <Btn v={salvoConsulta?"ghost":"primary"} disabled={salvoConsulta||salvandoNota} onClick={salvarProntuario} style={salvoConsulta?{}:{background:C.purple}}>{salvandoNota?<><Spin sz={13}/> ☁️ Salvando e enviando o laudo...</>:salvoConsulta?"✓ Salvo · laudo na nuvem":(editandoNotaId?"💾 Atualizar nota no prontuário":"💾 Salvar no prontuário do paciente")}</Btn>
                    <Btn v="ghost" onClick={imprimirLaudoConsulta}>🖨️ Imprimir laudo</Btn>
...
function DashClinica({pedidos,pacientes,clinicaObj,hideValues,onNavegar,planoIA,consultas}) {
  ...
  const retornos = (pacientes||[])
    ...
    .map(x=>{
      ...
      return { id:x.p.id, nome, dt:x.dt, dataTxt:x.dt.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}), status, zap };
    });
  return (
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
