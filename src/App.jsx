...
function Topbar({title,sub,onLogout,clinicaName,isAdmin,notifDestino,onMenu,isMobile,onNavegar,notifCtx}) {
  return (
          ...
          {aba==="plano"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {!plano ? (
                ...
                const corEtapa = (t) => t==="Reavaliação"?C.purple:t==="Palmilha"?C.green:C.accent;
                const iconEtapa = (t) => t==="Reavaliação"?"🔄":t==="Palmilha"?"🦶":"🩺";
                return (
                  <>
                      ...
                      {plano.etapas.map(e=>(
                        <Card key={e.id} p={13} style={{border:e.feito?`1px solid ${corEtapa(e.tipo)}40`:`1px solid ${C.border}`,background:e.feito?`${corEtapa(e.tipo)}07`:undefined}}>
                          <div style={{display:"flex",alignItems:"center",gap:11}}>
                            ...
                            <span style={{fontSize:16}}>{iconEtapa(e.tipo)}</span>
                            <div style={{flex:1}}>
...
function PacientesPage({pacientes,setPacientes,isAdmin,clinicaId,clinicaName,planoClinica,planoIA,pedidosReais}) {
...
function AppCore() {
  ...
  const renderPage = () => {
    ...
    if(userType==="clinica" || userType==="colaborador") {
      ...
      if(page==="pacientes")  return <PacientesPage pacientes={meusPacientes} setPacientes={setPacientes} isAdmin={false} clinicaId={clinicaId} clinicaName={clinicaName} planoClinica={planoClinica} planoIA={planoIAefetivo} pedidosReais={meusPedidos}/>;
    ...
    if(page==="pacientes")  return <PacientesPage pacientes={pacientes} setPacientes={setPacientes} isAdmin={true} clinicaId={null} clinicaName={null} planoClinica={"Enterprise"} planoIA={"admin"} pedidosReais={pedidos}/>;
