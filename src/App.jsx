    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        base.onload = () => {
          ...
          logo.onload = () => {
            // logo ocupa ~15% da largura, no canto inferior direito, discreto
            const lw = base.width * 0.15;
  ...
  async function gerarImagem() {
    ...
    try {
      const res = await fetch("/api/imagem", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: `${ESTILOS_IMG[imgEstilo] || ESTILOS_IMG["Ilustração"]}. Tema: ${imgPrompt}. Contexto: clínica de fisioterapia e palmilhas posturais 3D, ambiente saudável e profissional. IMPORTANTE: a imagem NÃO deve conter nenhum texto, palavra, letra, número ou logotipo — apenas a imagem visual limpa.` }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error((data && data.error && data.error.message) || ("Erro " + res.status));
      let imagemUrl = null;
