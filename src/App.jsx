...
{"queixa":"resumo objetivo","posturo":"análise postural detalhada","biomecanica":"análise biomecânica","diagnostico":"hipótese diagnóstica","conduta":"conduta proposta seguindo protocolo FisioPiede","prescricao_palmilha":{"tipo":"Inteira","flexibilidade":"Normal","cobertura":"EVA Perfurado","espessura":"4mm","observacoes_direito":"especificações pé direito com elementos do protocolo","observacoes_esquerdo":"especificações pé esquerdo com elementos do protocolo"},"exercicios":["exercício 1 do protocolo","exercício 2","exercício 3","exercício 4"],"retorno":"prazo","urgencia":"eletivo"}`;
  ...
  async function transcreverConsulta(){
    ...
    if(segs.length===0){ setErroAudio("Nenhum áudio capturado."); return; }
  ...
  // 💾 Salva (ou atualiza) a nota no prontuário do paciente
  const salvarProntuario = () => {
    ...
    setPacientes(prev => prev.map(p => {
      ...
      if(editandoNotaId){
        ...
        if(ix>=0) lista[ix] = { ...lista[ix], secoes:notaConsulta, transcricao: transConsulta || lista[ix].transcricao, editadoEm: new Date().toISOString() };
      } else {
        lista.push({ id:"NOTA-"+Date.now().toString(36), data:new Date().toISOString().split("T")[0], secoes:notaConsulta, transcricao:transConsulta });
      }
            ...
            <Card p={20} style={{border:`1px solid ${C.purple}30`,background:`${C.purple}06`}}>
              ...
              {notaConsulta&&(
                <div style={{marginTop:16,display:"flex",flexDirection:"column",gap:12}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    <Btn v={salvoConsulta?"ghost":"primary"} disabled={salvoConsulta} onClick={salvarProntuario} style={salvoConsulta?{}:{background:C.purple}}>{salvoConsulta?"✓ Salvo no prontuário do paciente":(editandoNotaId?"💾 Atualizar nota no prontuário":"💾 Salvar no prontuário do paciente")}</Btn>
                    <Btn v="ghost" onClick={()=>{setNotaConsulta(null);setTransConsulta("");setEditandoNotaId(null);setSalvoConsulta(false);setMostrarBruta(false);}}>🗑️ Descartar</Btn>
                  </div>
                </div>
              )}
