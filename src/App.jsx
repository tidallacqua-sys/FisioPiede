...
// Igual ao podeUsarIA, mas consome "qtd" unidades de uma vez (ex.: imagem = 30).
...
// Preço da palmilha por TIPO e forma de pagamento (antecipado x faturado)
// Tênis/Sapato: antecipado 250 / faturado 280  ·  Chinelo: antecipado 350 / faturado 380
...
const PAGAMENTO_OPCOES = {
  "antecipado": { label: "Pagar antecipado", antecipado: true  },
...
function NovoPedidoModal({onClose,onSave,pacientes,clinicaName,clinicaId}) {
  ...
  const save = () => {
    if(!f.paciente){ alert("Selecione o paciente."); return; }
    if(!f.produto){ alert("Selecione o tipo: Tênis, Sapato ou Chinelo."); return; }
    if(!pac.numeracao){ alert("⚠️ Este paciente não tem o NÚMERO DO CALÇADO cadastrado.\n\nEdite o cadastro do paciente e informe a numeração antes de fazer o pedido."); return; }
    if(filesDireito.length + filesEsquerdo.length === 0){ alert("⚠️ Anexe o arquivo da digitalização da palmilha (pé direito e/ou esquerdo) antes de enviar o pedido."); return; }
        ...
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:20}}>
          ...
          <div>
            ...
            {f.paciente&&(pac.numeracao
              ...
              : <div style={{marginTop:8,padding:"7px 11px",background:`${C.red}0C`,border:`1px solid ${C.red}33`,borderRadius:8,fontSize:11,color:C.red,fontWeight:600}}>⚠️ Este paciente não tem o número do calçado cadastrado. Edite o cadastro do paciente antes de fazer o pedido.</div>
            )}
          </div>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
              {[["Tênis","👟","R$ 280"],["Sapato","👞","R$ 280"],["Chinelo","🩴","R$ 350"]].map(([nome,ic,preco])=>(
...
function RemessaModal({cesta,onClose,onConfirm}) {
  const [pagamento,setPagamento] = useState("antecipado");
        ...
        <div style={{padding:18,display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {Object.entries(PAGAMENTO_OPCOES).map(([k,o])=>(
                <div key={k} onClick={()=>setPagamento(k)} style={{padding:"11px 13px",borderRadius:9,cursor:"pointer",border:`2px solid ${pagamento===k?C.accent:C.border}`,background:pagamento===k?`${C.accent}10`:"transparent"}}>
                  ...
                  <div style={{fontSize:10,fontWeight:600,color:C.muted,marginTop:3}}>{o.antecipado?"Tênis/Sapato 250 · Chinelo 350":"Tênis/Sapato 280 · Chinelo 380"}</div>
                </div>
              ))}
            </div>
...
{"queixa":"resumo objetivo","posturo":"análise postural detalhada","biomecanica":"análise biomecânica","diagnostico":"hipótese diagnóstica","conduta":"conduta proposta seguindo protocolo FisioPiede","prescricao_palmilha":{"tipo":"Inteira","flexibilidade":"Normal","cobertura":"EVA Perfurado","espessura":"4mm","observacoes_direito":"especificações pé direito com elementos do protocolo","observacoes_esquerdo":"especificações pé esquerdo com elementos do protocolo"},"exercicios":["exercício 1 do protocolo","exercício 2","exercício 3","exercício 4"],"retorno":"prazo","urgencia":"eletivo"}`;
  ...
  const MAX_SEG_AUDIO = 300; // ~5 minutos
  ...
  async function iniciarAudioConsulta(){
    ...
    try{
      ...
      mr.onstop = ()=>{ stream.getTracks().forEach(t=>t.stop()); enviarAudioParaTranscrever(); };
      mediaRecRef.current = mr;
      mr.start();
      setGravandoAudio(true); setTempoAudio(0);
      audioTimerRef.current = setInterval(()=>setTempoAudio(t=>t+1), 1000);
      // para sozinho ao atingir o limite de ~5 min
      audioStopTimerRef.current = setTimeout(()=>{ if(mediaRecRef.current && mediaRecRef.current.state!=="inactive"){ setErroAudio("Limite de 5 minutos atingido — transcrevendo o trecho gravado."); pararAudioConsulta(); } }, MAX_SEG_AUDIO*1000);
    }catch(e){
      setErroAudio(e && e.name==="NotAllowedError" ? "Permita o acesso ao microfone para gravar." : "Não foi possível iniciar a gravação de áudio.");
    }
  }
  ...
  async function enviarAudioParaTranscrever(){
    const blob = new Blob(chunksRef.current, { type:"audio/webm" });
    if(blob.size===0){ setErroAudio("Nenhum áudio capturado."); return; }
    const tamMB = blob.size/(1024*1024);
    if(tamMB>4){ setErroAudio(`O áudio ficou grande (${tamMB.toFixed(1)} MB). Grave trechos de até ~5 minutos.`); return; }
    setTranscrevendoAudio(true); setErroAudio("");
    try{
      const base64 = await new Promise((resolve,reject)=>{ const r=new FileReader(); r.onload=()=>resolve(String(r.result).split(",")[1]); r.onerror=()=>reject(new Error("falha ao ler áudio")); r.readAsDataURL(blob); });
      const res = await fetch("/api/transcrever",{ method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ audioBase64:base64, mime:"audio/webm", filename:"consulta.webm" }) });
      const data = await res.json();
      if(!res.ok) throw new Error((data&&data.error&&data.error.message)||("Erro "+res.status));
      const texto = (data.text||"").trim();
      if(texto) setTrans(prev => prev ? (prev.trim()+"\n"+texto) : texto);
      else setErroAudio("A transcrição voltou vazia. Tente falar mais perto do microfone.");
    }catch(e){
    ...
    <div style={{padding:20}}>
      ...
      {aba==="transcricao"&&(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          {/* Gravar consulta (áudio) → transcrição pelo Whisper */}
          {!ehIOS && (
            <Card p={20} style={{border:`1px solid ${C.purple}30`,background:`${C.purple}06`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
                <div>
                  <div style={{fontSize:14,fontWeight:800,color:C.purple}}>🎧 Gravar consulta (áudio → texto)</div>
                  <div style={{fontSize:11.5,color:C.muted,marginTop:3,lineHeight:1.5}}>Grava a conversa e transcreve com IA (Whisper). Ideal para trechos de até <strong>5 minutos</strong>. O texto aparece no campo acima.</div>
                </div>
                {!gravandoAudio
                  ? <Btn v="primary" disabled={transcrevendoAudio} onClick={iniciarAudioConsulta} style={{background:C.purple}}>{transcrevendoAudio?<><Spin sz={14}/> Transcrevendo...</>:"🎧 Iniciar gravação"}</Btn>
                  : <Btn v="danger" onClick={pararAudioConsulta}>⏹ Parar e transcrever ({fmtAudio})</Btn>
                }
              </div>
              {gravandoAudio&&<div style={{marginTop:12,padding:"10px 14px",background:`${C.red}0C`,border:`1px solid ${C.red}28`,borderRadius:9,display:"flex",alignItems:"center",gap:10}}><span style={{width:10,height:10,borderRadius:"50%",background:C.red,animation:"pulse 1.2s infinite"}}/><span style={{fontSize:12,color:C.sub}}>Gravando a consulta... <strong style={{fontFamily:"'Space Mono',monospace",color:C.red}}>{fmtAudio}</strong> — clique em "Parar e transcrever" quando terminar.</span></div>}
              {transcrevendoAudio&&<div style={{marginTop:12,fontSize:12,color:C.purple}}>Enviando o áudio para transcrição... isso pode levar alguns segundos.</div>}
              ...
              <div style={{marginTop:10,fontSize:10,color:C.muted,fontStyle:"italic"}}>⚠️ Grave somente com o consentimento do paciente (LGPD). A transcrição é um apoio e requer revisão.</div>
            </Card>
          )}
        </div>
      )}
...
function BackupCard() {
  ...
  const fazerBackup = async () => {
    ...
    try {
      const dump = { _info: "Backup FisioPiede", _data: new Date().toISOString(), dados: {} };
      // chaves simples
      const chaves = ["fp:clinicas", "fp:pacientes", "fp:consultas", "fp:solicitacoes", "fp:pagamentos", "fp:pedidos", "fp:pedidos:del"];
      for (const k of chaves) {
        try { dump.dados[k] = await LS.readAsync(k); } catch (e) { dump.dados[k] = null; }
      }
      // arquivos dos pedidos (imagens/STL) — cada um em chave própria
      const peds = dump.dados["fp:pedidos"] || [];
      dump.arquivos = {};
      for (const p of peds) {
        try { const arq = await LS.readAsync("fp:arq:" + p.id); if (arq) dump.arquivos["fp:arq:" + p.id] = arq; } catch (e) {}
      }
      // dados por paciente: dor e baropodometria já vão dentro de fp:pacientes/baropodometrias; a dor é separada:
      ...
      LS.write("fp:ultimoBackup", agora); setUltimo(agora);
      setMsg("✓ Backup baixado! Guarde o arquivo em local seguro (computador + nuvem).");
    } catch (e) {
    ...
    <Card p={20} style={{ border: `1px solid ${C.green}30`, background: `${C.green}06`, marginTop: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 11.5, color: C.muted, marginTop: 3, lineHeight: 1.5, maxWidth: 460 }}>Baixe uma cópia de segurança de todos os dados (clínicas, pacientes, pedidos, históricos). Faça isso regularmente e guarde o arquivo em local seguro.</div>
...
function NotificacoesPage({ destino, onNavegar }) {
  const key = "fp:notif:" + destino;
...
function BaropodometriaIA({ clinicaId, planoIA, pacientes, setPacientes, onUsoIA }) {
  ...
  const salvarHistorico = () => {
    ...
    setPacientes(prev => prev.map(p => {
      ...
      const baros = [...(p.baropodometrias || []), { id: "BARO-" + Date.now().toString(36), data: new Date().toISOString().split("T")[0], result }];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Card hover={false} p={20} style={{ background: `linear-gradient(135deg,${C.accent}06,${C.purple}04)`, border: `1px solid ${C.accent}18` }}>
        ...
        <Btn v="primary" full sz="lg" style={{ marginTop: 14 }} disabled={!img || loading} onClick={analisar}>{loading ? <><Spin sz={16} /> Analisando exame...{tentativaBaro > 1 ? ` (tentativa ${tentativaBaro})` : ""}</> : "✦ Analisar com IA"}</Btn>
      </Card>
      ...
      {result && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <Btn v={salvoBaro ? "ghost" : "primary"} sz="sm" style={{ flex: 1, justifyContent: "center" }} disabled={salvoBaro} onClick={salvarHistorico}>{salvoBaro ? "✓ Salvo no histórico" : "💾 Salvar no histórico"}</Btn>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12 }}>
            {historico.map((h, i) => (
              <div key={h.id || i} style={{ display: "flex", justifyContent: "space-between", padding: "7px 11px", background: `${C.bgGlass}`, border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12 }}>
                ...
                <span style={{ color: C.muted }}>{h.result?.diagnostico ? String(h.result.diagnostico).slice(0, 40) : ""}</span>
              </div>
            ))}
          </div>
          {historico.length >= 2 ? (
...
function MarketingPage({ clinicaName, clinicaObj, isAdmin, clinicaId, planoIA }) {
  ...
  const ESTILOS_IMG = {
    "Ilustração": "Ilustração vetorial moderna e sofisticada, estilo flat design premium, paleta de azuis (#3B82F6) e roxos (#6366F1) com tons claros, formas limpas, gradientes suaves, composição minimalista e elegante, qualidade de design de aplicativo profissional, fundo harmonioso, sem texto.",
    "Foto realista": "Fotografia profissional realista, alta definição, iluminação natural suave e cinematográfica, cores vibrantes e saudáveis, foco nítido, ambiente clean de clínica moderna, estética de campanha publicitária premium, sem texto sobreposto.",
    "Minimalista": "Design minimalista premium, muito espaço em branco, poucos elementos, paleta suave de azul e branco, linhas limpas, elegante e sofisticado, estilo editorial moderno de revista de saúde, sem texto.",
  };
  const salvarLogo = (e) => {
    const file = e.target.files?.[0]; if (!file) return;
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        base.onload = () => {
          ...
          logo.onload = () => {
            // logo ocupa ~22% da largura, no canto inferior direito, sobre um selo branco translúcido
            const lw = base.width * 0.22;
            const lh = logo.height * (lw / logo.width);
            const pad = base.width * 0.03;
            const x = base.width - lw - pad, y = base.height - lh - pad;
            ctx.save();
            ctx.globalAlpha = 0.92;
            ctx.fillStyle = "rgba(255,255,255,0.78)";
            const r = 12;
            const bx = x - 8, by = y - 8, bw = lw + 16, bh = lh + 16;
            ctx.beginPath();
            ctx.moveTo(bx + r, by); ctx.arcTo(bx + bw, by, bx + bw, by + bh, r);
            ctx.arcTo(bx + bw, by + bh, bx, by + bh, r); ctx.arcTo(bx, by + bh, bx, by, r);
            ctx.arcTo(bx, by, bx + bw, by, r); ctx.closePath(); ctx.fill();
            ctx.globalAlpha = 1;
  ...
  async function gerarImagem() {
    ...
    const permIA = podeUsarIAqtd(clinicaId, planoIA, 30);
    if (!permIA.ok) { setImgErro(permIA.msg); return; }
    setImgLoading(true); setImgErro(""); setImgResult(null);
    try {
      const res = await fetch("/api/imagem", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: `${imgPrompt}. Contexto: clínica de fisioterapia e palmilhas posturais 3D. ${ESTILOS_IMG[imgEstilo] || ESTILOS_IMG["Ilustração"]}` }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error((data && data.error && data.error.message) || ("Erro " + res.status));
      let imagemUrl = null;
      ...
      {aba === "ia" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <Card hover={false} p={20}>
            ...
            <textarea value={imgPrompt} onChange={e => setImgPrompt(e.target.value)} rows={3} placeholder="Ex: pessoa correndo feliz num parque ao amanhecer, destaque para os pés saudáveis" style={{ width: "100%", fontSize: 12, lineHeight: 1.6, resize: "vertical", marginBottom: 10 }} />
            <Btn v="primary" full disabled={imgLoading || !imgPrompt.trim()} onClick={gerarImagem} style={{ background: C.purple }}>{imgLoading ? <><Spin sz={14} /> Gerando imagem (pode levar ~15s)...</> : "🎨 Gerar imagem (30 análises)"}</Btn>
...
export default function App() {
  ...
  const renderPage = () => {
    ...
    if(page==="notificacoes") return <NotificacoesPage destino={"admin:master"} onNavegar={(pg)=>setPage(pg)}/>;
    if(page==="agenda")     return <AgendaPage consultas={clinicaId?consultas.filter(c=>c.clinicaId===clinicaId):consultas} setConsultas={setConsultas} pacientes={clinicaId?meusPacientes:pacientes} clinicaId={clinicaId} isAdmin={userType==="admin"}/>;
