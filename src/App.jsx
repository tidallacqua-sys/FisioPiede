...
const STORAGE_FP = {
  ...
  upload: async (file) => {
    ...
    try {
      const path = `pedidos/${Date.now()}_${Math.random().toString(36).slice(2,7)}_${STORAGE_FP.limparNome(file.name)}`;
      const r = await fetch(`${BACKEND.url}/storage/v1/object/${STORAGE_FP.bucket}/${path}`, {
        method: "POST",
        headers: { "apikey": BACKEND.key, "Authorization": "Bearer "+BACKEND.key, "Content-Type": file.type||"application/octet-stream", "x-upsert": "true" },
        body: file,
      });
      if(!r.ok) return null;
