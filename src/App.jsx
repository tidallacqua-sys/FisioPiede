...
function Sidebar({nav,active,setActive,userType,userName,onLogout,plano}) {
  ...
  return (
    <div style={{width:col?58:218,minHeight:"100vh",background:C.bgCard,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",transition:"width .25s",position:"sticky",top:0,flexShrink:0,zIndex:100}}>
      <div style={{padding:col?"15px 0":"15px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:col?"center":"space-between",gap:8}}>
        {!col&&<div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,borderRadius:8,background:`linear-gradient(135deg,${C.accent},${C.purple})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>🦶</div><span style={{fontWeight:900,fontSize:14}}>Fisio<span style={{color:C.accent}}>Piede</span></span></div>}
        {col&&<div style={{width:28,height:28,borderRadius:8,background:`linear-gradient(135deg,${C.accent},${C.purple})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>🦶</div>}
        <button onClick={()=>setCol(!col)} style={{background:"none",color:C.muted,fontSize:12,padding:"3px 5px",borderRadius:5}}>{col?"›":"‹"}</button>
      </div>
      {!col&&<div style={{padding:"7px 13px",display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}><span style={{fontSize:9,fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",color:tc,background:`${tc}14`,padding:"3px 8px",borderRadius:4}}>{tl}</span>{userType==="clinica"&&plano&&<span style={{fontSize:9,fontWeight:700,textTransform:"uppercase",color:plano==="Enterprise"?C.gold:plano==="Premium"?C.purple:C.muted,background:`${plano==="Enterprise"?C.gold:plano==="Premium"?C.purple:C.muted}14`,padding:"3px 8px",borderRadius:4}}>{plano==="Enterprise"?"👑 ":plano==="Premium"?"💎 ":""}{plano}</span>}</div>}
      <nav style={{flex:1,padding:"5px 0",overflowY:"auto"}}>
        {nav.map(item=>{
          const a = active===item.id;
          return (
            <button key={item.id} onClick={()=>setActive(item.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:col?"10px 0":"10px 13px",justifyContent:col?"center":"flex-start",background:a?`${C.accent}13`:"transparent",color:a?C.soft:C.muted,fontSize:13,fontWeight:a?600:400,borderLeft:a?`2px solid ${C.accent}`:"2px solid transparent",transition:"all .13s"}}>
              <span style={{fontSize:15}}>{item.icon}</span>{!col&&<span>{item.label}</span>}
            </button>
          );
        })}
      </nav>
      <div style={{padding:col?"10px 0":"10px 13px",borderTop:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:8,justifyContent:col?"center":"flex-start"}}>
        ...
        {!col&&<div style={{flex:1,minWidth:0}}><div style={{fontSize:11,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{userName}</div><button onClick={onLogout} style={{background:"none",color:C.muted,fontSize:10,padding:0}}>Sair →</button></div>}
      </div>
    </div>
  );
}

function Topbar({title,sub,onLogout,clinicaName,isAdmin,notifDestino}) {
  return (
    <div style={{height:56,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",background:`${C.bgCard}dd`,backdropFilter:"blur(14px)",position:"sticky",top:0,zIndex:50}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        {!isAdmin&&clinicaName&&<div style={{padding:"3px 10px",background:`${C.green}14`,border:`1px solid ${C.green}30`,borderRadius:99,fontSize:10,fontWeight:700,color:C.green}}>🏥 {clinicaName}</div>}
        {isAdmin&&<div style={{padding:"3px 10px",background:`${C.accent}14`,border:`1px solid ${C.accent}30`,borderRadius:99,fontSize:10,fontWeight:700,color:C.accent}}>⚡ Admin Master</div>}
        <div><div style={{fontWeight:800,fontSize:14}}>{title}</div>{sub&&<div style={{fontSize:11,color:C.muted}}>{sub}</div>}</div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:11,color:C.muted,display:"flex",alignItems:"center",gap:5}}><span style={{width:6,height:6,borderRadius:"50%",background:C.green,display:"inline-block"}}/>Online</span>
        {notifDestino&&<NotifBell destino={notifDestino}/>}
        <Btn v="ghost" sz="sm" onClick={onLogout}>Sair</Btn>
      </div>
    </div>
  );
}
...
function Login({onLogin,clinicas,pacientes}) {
  ...
  const enviarSolicitacao = () => {
    ...
    try {
      ...
      pushNotif("admin:master", "📩", "Nova solicitação de acesso", `${solic.clinica} (${solic.cidade||"—"}) solicitou licença.`);
    } catch(e){}
    setSent(true);
  };
  ...
  const handle = () => {
    ...
    setTimeout(()=>{
      ...
      // Clínica — verifica nas clínicas cadastradas
      const clinica = clinicas.find(c=>c.email===email.trim()&&c.senha===pass);
      if(clinica){
...
function DashAdmin({pedidos,clinicas}) {
  ...
  const inadimp = clinicas.filter(c=>c.status==="Inadimplente").length;
  return (
...
function NovoPedidoModal({onClose,onSave,pacientes,clinicaName,clinicaId}) {
  ...
  const save = () => {
    ...
    onSave({id,clinicaId:clinicaId||null,clinica:clinicaName||"—",paciente:f.paciente,tipo:f.tipoPalmilha,status:"Recebido",data:new Date().toISOString().split("T")[0],rastreio:"",updatedAt:t,log:[`${t} — Recebido`],peso:pac.peso||"",altura:pac.altura||"",numeracao:pac.numeracao||"",arquivos,...f});
    onClose();
  };
    ...
    <Modal onClose={onClose}>
      <Card hover={false} p={0} style={{width:"100%",maxWidth:720,maxHeight:"93vh",overflowY:"auto",animation:"fadeUp .25s ease"}}>
        ...
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:20}}>
          ...
          {/* Valor */}
          <div style={{padding:14,background:`${C.green}07`,border:`1px solid ${C.green}22`,borderRadius:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:12,fontWeight:700,color:C.text}}>Valor deste pedido</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>Palmilha personalizada · produção 3D</div>
            </div>
            <div style={{fontSize:24,fontWeight:900,color:C.green}}>R$ {brl(PRECO)}</div>
          </div>
...
function PedidosPage({pedidos,setPedidos,isAdmin,clinicaId,clinicaName,pacientes,onRecarregar}) {
  ...
  const clOpts = ["Todas",...new Set(pedidos.map(p=>p.clinica))];
...
function ClinicasPage({clinicas,setClinicas}) {
  ...
  const emptyF = {nome:"",cnpj:"",cpf:"",resp:"",email:"",tel:"",cep:"",rua:"",numero:"",complemento:"",bairro:"",cidade:"",estado:"",plano:"Básico",status:"Ativa",senha:"",confirmarSenha:""};
  const [f,setF] = useState(emptyF);
  const sf = (k,v) => { setF(prev=>({...prev,[k]:v})); setFormErr(""); };

  const vis = clinicas.filter(c=>c.nome.toLowerCase().includes(search.toLowerCase()));
  ...
  const save = () => {
    ...
    const emailExiste = clinicas.find(c=>c.email===f.email.trim()&&c.id!==edit?.id);
    ...
    if(edit) {
      ...
      setClinicas(p=>[...p,{id:Date.now(),pedidos:0,...f}]);
    }
  ...
  return (
    <div style={{padding:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
        {[{l:"Total",v:clinicas.length,c:C.accent},{l:"Ativas",v:clinicas.filter(c=>c.status==="Ativa").length,c:C.green},{l:"Inadimp.",v:clinicas.filter(c=>c.status==="Inadimplente").length,c:C.amber},{l:"Bloqueadas",v:clinicas.filter(c=>c.status==="Bloqueada").length,c:C.red}].map((s,i)=>(
                  ...
                  {vis.map(c=>(
                    <React.Fragment key={c.id}>
                      <tr style={{borderBottom:expandedId===c.id?"none":`1px solid ${C.border}`,transition:"background .12s",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,.02)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"} onClick={()=>setExpandedId(expandedId===c.id?null:c.id)}>
                        <td style={{padding:"11px 14px"}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:28,height:28,borderRadius:7,background:`linear-gradient(135deg,${C.accent}30,${C.purple}30)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>🏥</div><div><div style={{fontWeight:700}}>{c.nome}</div>{(c.cidade||c.estado)&&<div style={{fontSize:10,color:C.muted}}>{[c.cidade,c.estado].filter(Boolean).join("/")}</div>}</div></div></td>
                        ...
                        <td style={{padding:"11px 14px"}}><Badge label={c.status} color={c.status==="Ativa"?C.green:c.status==="Inadimplente"?C.amber:C.red}/></td>
                        <td style={{padding:"11px 14px"}}>
                          <div style={{display:"flex",gap:4}}>
                            <Btn v="ghost" sz="sm" onClick={e=>{e.stopPropagation();openEdit(c);}}>✏️</Btn>
                            <Btn v="ghost" sz="sm" onClick={e=>{e.stopPropagation();toggle(c.id,"status",c.status==="Bloqueada"?"Ativa":"Bloqueada");}}>{c.status==="Bloqueada"?"🔓":"🔒"}</Btn>
            ...
            <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
              ...
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  ...
                  <div><label>Plano</label><select value={f.plano||"Básico"} onChange={e=>sf("plano",e.target.value)}><option>Básico</option><option>Premium</option><option>Enterprise</option></select></div>
                  <div><label>Status</label><select value={f.status||"Ativa"} onChange={e=>sf("status",e.target.value)}><option>Ativa</option><option>Inadimplente</option><option>Bloqueada</option></select></div>
                </div>
              </div>
...
function FinanceiroPage({clinicas,isAdmin,clinicaObj,meusPedidos}) {
  ...
  const list = isAdmin ? clinicas : (clinicaObj ? [{...clinicaObj, pedidos:clinicaPedidos}] : []);
  ...
  const get  = c => { const np = c?.pedidosReal ?? c?.pedidos ?? 0; return {np, fat:np*PRECO, pago:pagos[c?.nome]?true:c?.status!=="Inadimplente"}; };
...
// ─── PACIENTES ──────────────────────────────────────────────────────────────────
function PacientesPage({pacientes,setPacientes,isAdmin,clinicaId,clinicaName}) {
  const [search,setSearch]   = useState("");
        ...
        {vis.map((p,i)=>(
          <Card key={p.id} style={{padding:16,animation:`fadeUp .4s ${i*.04}s ease both`,opacity:0}}>
            ...
            <div style={{display:"flex",gap:5}}>
              ...
              <Btn v="ghost"   sz="sm" style={{flex:1,justifyContent:"center"}}>+ Pedido</Btn>
            </div>
          </Card>
        ))}
      </div>
...
function IAPage({pacientes, setPacientes, setPedidos, clinicaId, clinicaName}) {
...
function DashClinica({pedidos,pacientes,clinicaObj,hideValues}) {
    ...
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      ...
      <Card hover={false} p={24} style={{background:`linear-gradient(135deg,${C.accent}08,${C.purple}05)`,border:`1px solid ${C.accent}20`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}><Btn v="primary">💳 Pagar via PIX</Btn><Btn v="ghost">📄 Boleto</Btn></div>
        </div>
      </Card>
      )}
...
// ══════════════════════════════════════════════════════════════════════════════
function AcademyPage({ userName, userType, clinicaName }) {
...
// NOTIFICAÇÕES
// ══════════════════════════════════════════════════════════════════════════════
function pushNotif(destino, icon, titulo, texto) {
  try {
    const key = "fp:notif:" + destino;
    const atual = LS.read(key) || [];
    const nova = { id: Date.now() + Math.random(), icon, titulo, texto, ts: Date.now(), lida: false };
    LS.write(key, [nova, ...atual].slice(0, 50));
  } catch (e) {}
}

function NotifBell({ destino }) {
  const key = "fp:notif:" + destino;
        ...
        <>
          ...
          <div style={{ position: "absolute", top: 36, right: 0, width: 320, maxHeight: 420, overflowY: "auto", background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 12, boxShadow: "0 12px 40px rgba(0,0,0,.4)", zIndex: 91 }}>
            ...
            {notifs.length === 0 ? (
              ...
              <div key={n.id} style={{ padding: "11px 14px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 10, background: n.lida ? "transparent" : `${C.accent}06` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 9, color: C.muted, marginTop: 3 }}>{new Date(n.ts).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</div>
                </div>
...
function ChatBox({ pacienteId, remetente, nomePaciente, nomeClinica }) {
  ...
  const enviar = () => {
    ...
    pushNotif(destino, "💬", `Nova mensagem de ${remetente === "paciente" ? nomePaciente : nomeClinica}`, input.trim().slice(0, 40));
    setInput("");
  };
...
// ══════════════════════════════════════════════════════════════════════════════
function MarketingPage({ clinicaName, clinicaObj, isAdmin }) {
  const [aba, setAba] = useState("dashboard");
...
function UpgradePlano({ plano, modulo }) {
  ...
  const PLANOS = [
    { nome:"Básico", cor:"#64748B", icon:"🟢", recursos:["Dashboard","Pedidos","Avaliação","Pacientes"] },
    { nome:"Premium", cor:"#8B5CF6", icon:"💎", recursos:["Tudo do Básico","Agenda","Financeiro"] },
    { nome:"Enterprise", cor:"#F59E0B", icon:"👑", recursos:["Tudo do Premium","Inteligência Artificial","Academy","Marketing Hub"] },
  ];
  return (
    <div style={{ padding: 20 }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <Card hover={false} p={32} style={{ textAlign: "center", background: `linear-gradient(135deg,${C.purple}10,${C.accent}06)`, border: `1px solid ${C.purple}25`, marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 900, marginBottom: 8 }}>Módulo {nomesModulo[modulo] || modulo} indisponível no seu plano</div>
          <div style={{ fontSize: 13, color: C.sub, lineHeight: 1.7, maxWidth: 460, margin: "0 auto" }}>Seu plano atual é o <strong style={{ color: C.text }}>{plano}</strong>. Faça upgrade para desbloquear este e outros recursos e potencializar sua clínica.</div>
        </Card>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          {PLANOS.map(p => { const atual = p.nome === plano; return (
        ...
        <div style={{ textAlign: "center", marginTop: 20 }}>
          <div style={{ fontSize: 12, color: C.muted }}>Para fazer upgrade, entre em contato com a equipe FisioPiede.</div>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [clinicas,   _setClinicas]   = useState(() => LS.read("fp:clinicas")  || [SEED_CLINICA]);
  const [pedidos,    _setPedidos]    = useState(() => LS.read("fp:pedidos")   || [SEED_PEDIDO]);
  const [pacientes,  _setPacientes]  = useState(() => LS.read("fp:pacientes") || [SEED_PACIENTE]);
  const [consultas,  _setConsultas]  = useState(() => LS.read("fp:consultas") || [SEED_CONSULTA]);

  // Wrappers: atualizam React state E gravam no storage (window.storage + localStorage) imediatamente
  const setClinicas  = (u) => _setClinicas(p  => { const n = typeof u==='function'?u(p):u; LS.write("fp:clinicas",  n); return n; });
  const setPedidos   = (u) => _setPedidos(p   => { const n = typeof u==='function'?u(p):u; LS.write("fp:pedidos",   n); return n; });
  const setPacientes = (u) => _setPacientes(p => { const n = typeof u==='function'?u(p):u; LS.write("fp:pacientes", n); return n; });
  const setConsultas = (u) => _setConsultas(p => { const n = typeof u==='function'?u(p):u; LS.write("fp:consultas", n); return n; });
      ...
      try {
        ...
        if(pac){
          ...
          if(LS.read(flagKey)!==hoje){
            pushNotif(destino,"💪","Hora dos exercícios!","Não esqueça de fazer seus exercícios de hoje.");
            if(pac.dataRetorno){
              const dias = Math.ceil((new Date(pac.dataRetorno)-new Date())/86400000);
              if(dias>=0&&dias<=3) pushNotif(destino,"📅","Consulta de retorno próxima",`Seu retorno é em ${dias===0?"hoje":dias+" dia(s)"} (${pac.dataRetorno.split("-").reverse().join("/")}).`);
            }
  ...
  const planoClinica = (clinicaObj && clinicaObj.plano) || "Básico";
  ...
  // Filtra o menu da clínica conforme o plano
  if (ehClinica && modulosPermitidos) {
    nav = NAV_CLINICA.filter(item => modulosPermitidos.includes(item.id));
  }
  // Colaborador: remove o Financeiro do menu (acesso restrito)
  if (userType==="colaborador") {
    nav = nav.filter(item => item.id !== "financeiro");
  }
  ...
  const renderPage = () => {
    ...
    if(userType==="clinica" || userType==="colaborador") {
      ...
      if(modulosPermitidos && !modulosPermitidos.includes(page)) {
        return <UpgradePlano plano={planoClinica} modulo={page}/>;
      }
      if(page==="dashboard")  return <DashClinica pedidos={meusPedidos} pacientes={meusPacientes} clinicaObj={clinicaObj} hideValues={ehColab}/>;
      ...
      if(page==="pacientes")  return <PacientesPage pacientes={meusPacientes} setPacientes={setPacientes} isAdmin={false} clinicaId={clinicaId} clinicaName={clinicaName}/>;
      ...
      if(page==="ia")         return <IAPage pacientes={meusPacientes} setPacientes={setPacientes} setPedidos={setPedidos} clinicaId={clinicaId} clinicaName={clinicaName}/>;
      if(page==="academy")    return <AcademyPage userName={userName} userType={userType} clinicaName={clinicaName}/>;
      if(page==="marketing")  return <MarketingPage clinicaName={clinicaName} clinicaObj={clinicaObj} isAdmin={false}/>;
    ...
    if(page==="agenda")     return <AgendaPage consultas={clinicaId?consultas.filter(c=>c.clinicaId===clinicaId):consultas} setConsultas={setConsultas} pacientes={clinicaId?meusPacientes:pacientes} clinicaId={clinicaId} isAdmin={userType==="admin"}/>;
    if(page==="pacientes")  return <PacientesPage pacientes={pacientes} setPacientes={setPacientes} isAdmin={true} clinicaId={null} clinicaName={null}/>;
    if(page==="ia")         return <IAPage pacientes={pacientes} setPacientes={setPacientes} setPedidos={setPedidos} clinicaId={null} clinicaName={null}/>;
    if(page==="academy")    return <AcademyPage userName={userName} userType={userType} clinicaName={clinicaName}/>;
    if(page==="marketing")  return <MarketingPage clinicaName={null} clinicaObj={null} isAdmin={true}/>;
    ...
    <div>
      ...
      {screen==="app" && (
        <div style={{display:"flex",minHeight:"100vh"}}>
          <Sidebar nav={nav} active={page} setActive={setPage} userType={userType} userName={clinicaName||userName} onLogout={logout} plano={userType==="clinica"?planoClinica:null}/>
          <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
            <Topbar title={TITLES[page]||page} sub={userEmail} clinicaName={clinicaName} isAdmin={userType==="admin"} onLogout={logout} notifDestino={userType==="paciente"?("paciente:"+pacienteId):(userType==="clinica"||userType==="colaborador")?("clinica:"+clinicaId):"admin:master"}/>
