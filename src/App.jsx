...
function Sidebar({nav,active,setActive,userType,userName,onLogout,plano,bloqueados}) {
    ...
    <div style={{width:col?58:218,minHeight:"100vh",background:C.bgCard,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",transition:"width .25s",position:"sticky",top:0,flexShrink:0,zIndex:100}}>
      ...
      <nav style={{flex:1,padding:"5px 0",overflowY:"auto"}}>
        {nav.map(item=>{
          ...
          return (
            <button key={item.id} onClick={()=>setActive(item.id)} title={isBloq?"Recurso bloqueado — faça upgrade do plano":item.label} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:col?"10px 0":"10px 13px",justifyContent:col?"center":"flex-start",background:a?`${C.accent}13`:"transparent",color:isBloq?C.muted+"99":(a?C.soft:C.muted),fontSize:13,fontWeight:a?600:400,borderLeft:a?`2px solid ${C.accent}`:"2px solid transparent",transition:"all .13s",opacity:isBloq?0.78:1}}>
              <span style={{fontSize:15}}>{isBloq?"🔒":item.icon}</span>{!col&&<span style={{flex:1,textAlign:"left"}}>{item.label}</span>}{!col&&isBloq&&<span style={{fontSize:8,fontWeight:800,color:C.purple,background:`${C.purple}1a`,padding:"2px 5px",borderRadius:4,textTransform:"uppercase",letterSpacing:".03em"}}>PRO</span>}
            </button>
          );
        })}
      </nav>
...
export default function App() {
  ...
  if (ehClinica && modulosPermitidos) {
    modulosBloqueados = NAV_CLINICA.filter(item => !modulosPermitidos.includes(item.id)).map(item => item.id);
  }
    ...
    <div>
      ...
      {screen==="app" && (
        <div style={{display:"flex",minHeight:"100vh"}}>
          <Sidebar nav={nav} active={page} setActive={setPage} userType={userType} userName={clinicaName||userName} onLogout={logout} plano={userType==="clinica"?planoClinica:null} bloqueados={modulosBloqueados}/>
