...
function Login({onLogin,clinicas,pacientes}) {
  ...
  const handle = () => {
    ...
    setTimeout(async ()=>{
      ...
      const colab = todosColab.find(c=>c.usuario===email.trim()&&c.senha===pass);
      if(colab){
...
function UploadZone({label,side,files,setFiles}) {
  ...
  const handleFiles = async (fileList) => {
    const LIMITE_MB = 3.5; // limite seguro do armazenamento atual (acima disso o arquivo chega corrompido)
    ...
    // bloqueia os grandes ANTES de tentar salvar (senão chegam "sem dados")
    const grandes = extOk.filter(f => f.size/1024/1024 > LIMITE_MB);
    const ok = extOk.filter(f => f.size/1024/1024 <= LIMITE_MB);
    if(grandes.length>0){
      const nomes = grandes.map(f => `${f.name} (${(f.size/1024/1024).toFixed(1)}MB)`).join(", ");
      setAvisoArq({ nomes, whats: WHATS_FP });
    }
    if(ok.length===0) return;
    const mapped = await Promise.all(ok.map(async file => {
      let dataUrl = null;
    ...
    <div>
      ...
      >
        ...
        {files.length===0 && (
          <>
            ...
            <div style={{fontSize:10,color:C.muted}}>OBJ · STL · JPG — máx. 3,5MB por arquivo</div>
          </>
        )}
        {files.length>0 && (
      ...
      {avisoArq && (
        <div style={{marginTop:8,padding:"11px 13px",background:`${C.amber}10`,border:`1px solid ${C.amber}40`,borderRadius:9}}>
          <div style={{fontSize:11.5,color:C.amber,fontWeight:700,marginBottom:4}}>⚠️ Arquivo grande demais para envio aqui</div>
          <div style={{fontSize:11,color:C.sub,lineHeight:1.5,marginBottom:8}}>O arquivo <strong>{avisoArq.nomes}</strong> passou do limite de 3,5MB e não pôde ser anexado com segurança. Para não travar o pedido, envie este arquivo .obj/.stl diretamente para o FisioPiede pelo WhatsApp, citando o nome do paciente.</div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <a href={`https://wa.me/${avisoArq.whats}?text=${encodeURIComponent("Olá! Preciso enviar um arquivo 3D grande de um pedido (acima de 3,5MB): "+avisoArq.nomes)}`} target="_blank" rel="noreferrer" style={{textDecoration:"none"}}><Btn v="success" sz="sm">📱 Enviar por WhatsApp</Btn></a>
...
function NovoPedidoModal({onClose,onSave,pacientes,clinicaName,clinicaId}) {
  ...
  const save = () => {
    ...
    if(!numEfetivo){ alert("⚠️ Informe o NÚMERO DO CALÇADO do paciente para concluir o pedido."); return; }
    ...
    onSave({id,clinicaId:clinicaId||null,clinica:clinicaName||"—",paciente:f.paciente,tipo:f.tipoPalmilha,status:"Recebido",data:new Date().toISOString().split("T")[0],rastreio:"",updatedAt:t,log:[`${t} — Recebido`],peso:pac.peso||"",altura:pac.altura||"",numeracao:numEfetivo,arquivos,remessaId:null,enviado:false,...f});
    onClose();
  };
              ...
              {clinicas.map((c,i)=>{ const conv=getConv(c.nome); const cred=conv*2; return (
                <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,padding:"10px 12px",background:C.bgGlass,border:`1px solid ${C.border}`,borderRadius:9,flexWrap:"wrap"}}>
                  <div style={{minWidth:140,flex:1}}>
                    ...
                    <div style={{fontSize:10.5,color:C.muted}}>{conv} indicação(ões) · <span style={{color:C.green,fontWeight:700}}>{cred} créditos de palmilha</span></div>
                  </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Card hover={false} p={20} style={{ background: `linear-gradient(135deg,${C.accent}08,${C.purple}06)`, border: `1px solid ${C.accent}22` }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <Btn v="primary" sz="sm" onClick={() => copiar(`Conheça a FisioPiede e ative seu Plano Enterprise! Use meu código ${codigoElite}. 👑🦶`, "elitecod")}>{copiado === "elitecod" ? "✓ Copiado!" : "📋 Copiar convite"}</Btn>
            </div>
            <div style={{ marginTop: 14, fontSize: 11.5, color: C.sub, lineHeight: 1.7 }}>
              <strong style={{ color: C.text }}>Como funciona:</strong><br/>
              1️⃣ Compartilhe seu código com colegas de outras clínicas<br/>
