...
function FechamentoPage({clinicas,pedidos}) {
  ...
  const precoProd = (p) => /hinelo/i.test(p.produto||p.tipoProduto||p.tipo||"") ? 350 : 280;
  const rows = clinicas.map(c=>{
    const peds = pedidos ? pedidos.filter(p=>p.clinicaId===c.id) : [];
    const np = pedidos ? peds.length : c.pedidos;
    const fat = pedidos ? peds.reduce((a,p)=>a+precoProd(p),0) : np*PRECO;
    const remessas = {};
  ...
  const abrirRelatorio = (titulo, corpoHtml) => {
    ...
    const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>${esc(titulo)}</title>
      <style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#0F172A;padding:36px 40px;font-size:13px;line-height:1.6;}
      .top{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #3B82F6;padding-bottom:14px;margin-bottom:20px;}
      .marca{font-size:23px;font-weight:800;color:#3B82F6;}.marca span{color:#0F172A;}.sub{font-size:10px;color:#64748B;letter-spacing:.05em;}
      h1{font-size:16px;margin-bottom:3px;}table{width:100%;border-collapse:collapse;margin:8px 0;}th,td{padding:7px 9px;border-bottom:1px solid #E2E8F0;text-align:left;}
      th{font-size:9px;text-transform:uppercase;letter-spacing:.06em;color:#64748B;}.tot{font-weight:800;border-top:2px solid #CBD5E1;}
      .bt{font-size:10px;font-weight:800;text-transform:uppercase;color:#3B82F6;margin:18px 0 6px;border-left:3px solid #3B82F6;padding-left:8px;}
      .foot{margin-top:26px;border-top:1px solid #E2E8F0;padding-top:12px;font-size:10px;color:#94A3B8;text-align:center;}
      @media print{body{padding:22px 26px;}button{display:none;}}</style></head><body>
      <div class="top"><div><div class="marca">Fisio<span>Piede</span></div><div class="sub">HEALTH TECH PLATFORM</div></div>
      <div style="text-align:right;font-size:12px;color:#334155;">Fechamento ${esc(mes)}<br>Emitido em ${new Date().toLocaleDateString("pt-BR")}</div></div>
      ${corpoHtml}
      <div class="foot">FisioPiede Health Tech Platform • Documento de fechamento financeiro.</div>
      <script>window.onload=function(){setTimeout(function(){window.print();},350);};<\/script></body></html>`;
    w.document.open(); w.document.write(html); w.document.close();
  };

  const relatorioClinica = (r) => {
    const pacRows = (r.peds||[]).map(p=>`<tr><td>${p.paciente||"(sem nome)"}</td><td>${p.produto||p.tipoProduto||(/hinelo/i.test(p.tipo||"")?"Chinelo":"Palmilha")}</td><td>R$ ${brl(precoProd(p))}</td></tr>`).join("");
    const freteRows = (r.fretesList||[]).map(fr=>`<tr><td>${fr.label}</td><td>${fr.data||"—"}</td><td>R$ ${brl(fr.valor)}</td></tr>`).join("") || `<tr><td colspan="3">Sem fretes no período</td></tr>`;
    abrirRelatorio(`Fechamento — ${r.nome}`, `
      <h1>${r.nome}</h1>
      <div style="font-size:12px;color:#475569;margin-bottom:6px;">${r.np} palmilha(s) no período</div>
      <div class="bt">Pacientes & Palmilhas</div>
      <table><thead><tr><th>Paciente</th><th>Tipo</th><th>Valor</th></tr></thead><tbody>${pacRows}
        <tr class="tot"><td>Subtotal palmilhas</td><td></td><td>R$ ${brl(r.fat)}</td></tr></tbody></table>
      <div class="bt">Fretes por remessa</div>
      <table><thead><tr><th>Frete</th><th>Data</th><th>Valor</th></tr></thead><tbody>${freteRows}
        <tr class="tot"><td>Subtotal fretes</td><td></td><td>R$ ${brl(r.frete)}</td></tr></tbody></table>
      <table><tbody><tr class="tot" style="font-size:15px;"><td>TOTAL A PAGAR</td><td></td><td style="color:#059669;">R$ ${brl(r.totalCobrar)}</td></tr></tbody></table>
      <div style="margin-top:14px;font-size:12px;">Pagamento via PIX — chave: <b>${PIX_FP}</b></div>`);
  };
  ...
  const relatorioTotal = () => {
    ...
    abrirRelatorio(`Fechamento Total — ${mes}`, `
    ...
    <div style={{padding:20}}>
      ...
      {gerado&&(
        <div>
          ...
          <Card hover={false} p={0} style={{overflow:"hidden"}}>
            <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontWeight:800}}>Fechamento Detalhado — {mes}</div><div style={{display:"flex",gap:7}}><Btn v="gold" sz="sm" onClick={relatorioTotal}>📄 Relatório Total</Btn></div></div>
...
function BaropodometriaIA({ clinicaId, planoIA }) {
  ...
  const onFile = (e) => {
    ...
    const r = new FileReader();
    r.onload = () => { setImg({ dataUrl: r.result, nome: file.name, media: file.type, ehPdf }); setResult(null); setErro(""); };
    r.readAsDataURL(file);
  };
