...
const PLANOS_INFO = [
  ...
  { nome:"Enterprise", cor:"#F59E0B", icon:"👑", preco:119.90, precoLabel:"12x R$ 119,90/ano", iaUsos:60, recursos:["Tudo do Premium","IA Clínica completa","Academy","Marketing Hub","60 análises de IA/mês"] },
];
// Limite de usos de IA por mês conforme plano (trial = 10)
const IA_LIMITE = { "Básico":0, "Premium":30, "Enterprise":60, "Trial":10 };

function UpgradePlano({ plano, modulo, clinicaNome }) {
  ...
  const assinar = (p) => {
    const msg = `Olá! Sou da clínica ${clinicaNome||""} e quero assinar o plano ${p.nome} (${p.precoLabel}) da FisioPiede.`;
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          {PLANOS_INFO.map(p => { const atual = p.nome === plano; return (
            <Card key={p.nome} p={20} style={{ border: atual ? `2px solid ${p.cor}` : `1px solid ${C.border}`, position: "relative" }}>
              ...
              {!atual && p.preco > 0 && <div style={{ marginTop: 14 }}><Btn v="primary" full onClick={() => assinar(p)}>Assinar</Btn></div>}
            </Card>
          ); })}
        </div>
        <div style={{ textAlign: "center", marginTop: 20 }}>
          <div style={{ fontSize: 12, color: C.muted }}>Ao clicar em "Assinar", você fala com nossa equipe para ativar o plano. A liberação é feita após a confirmação do pagamento.</div>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // Trial de 2 dias: libera Enterprise; depois cai para o plano contratado
  const TRIAL_DIAS = 2;
  const planoContratado = (clinicaObj && clinicaObj.plano) || "Básico";
  let emTrial = false, diasRestantesTrial = 0;
  if (clinicaObj && clinicaObj.trialInicio) {
    const inicio = new Date(clinicaObj.trialInicio).getTime();
    const agora = Date.now();
    const diasPassados = (agora - inicio) / (1000 * 60 * 60 * 24);
    if (diasPassados < TRIAL_DIAS) { emTrial = true; diasRestantesTrial = Math.ceil(TRIAL_DIAS - diasPassados); }
  }
  // Durante o trial, acesso Enterprise (tudo). Depois, o plano contratado.
  const planoClinica = emTrial ? "Enterprise" : planoContratado;
  ...
  const renderPage = () => {
    ...
    if(userType==="clinica" || userType==="colaborador") {
      ...
      if(modulosPermitidos && !modulosPermitidos.includes(page)) {
        return <UpgradePlano plano={planoClinica} modulo={page} clinicaNome={clinicaName}/>;
      }
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
            ...
            {userType==="clinica" && emTrial && (
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"10px 16px",background:"linear-gradient(90deg,#8B5CF615,#F59E0B15)",borderBottom:"1px solid #8B5CF630",fontSize:13}}>
                ...
                <Btn v="primary" sz="sm" onClick={()=>setPage("config")} style={{marginLeft:6}}>Ver planos</Btn>
              </div>
            )}
