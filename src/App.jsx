...
// Igual ao podeUsarIA, mas consome "qtd" unidades de uma vez (imagem/transcrição = 10, baropodômetro = 5).
    ...
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:18}}>
      ...
      {(()=>{
        ...
        const radar = (clinicas||[]).map(c=>{
          ...
          const limite = (IA_LIMITE[plano]!==undefined?IA_LIMITE[plano]:0) + creditoExtraIA(c.id);
          const uso = IA_USO.atual(c.id);
...
function NovoPedidoModal({onClose,onSave,pacientes,clinicaName,clinicaId}) {
  ...
  const montarPedido = () => {
    ...
    return {id,clinicaId:clinicaId||null,clinica:clinicaName||"—",paciente:f.paciente,tipo:f.tipoPalmilha,status:"Recebido",data:new Date().toISOString().split("T")[0],rastreio:"",updatedAt:t,log:[`${t} — Recebido`],peso:pac.peso||"",altura:pac.altura||"",numeracao:numEfetivo,arquivos,remessaId:null,enviado:false,...f};
  };
  const save = () => {
...
function ClinicasPage({clinicas,setClinicas}) {
  ...
  const save = () => {
    ...
    if(edit) {
      ...
      setClinicas(p=>[...p,{id:Date.now(),pedidos:0,trialInicio:new Date().toISOString(),...f,...heranca}]);
...
{"queixa":"resumo objetivo","posturo":"análise postural detalhada","biomecanica":"análise biomecânica","diagnostico":"hipótese diagnóstica","conduta":"conduta proposta seguindo protocolo FisioPiede","prescricao_palmilha":{"tipo":"Inteira","flexibilidade":"Normal","cobertura":"EVA Perfurado","espessura":"4mm","observacoes_direito":"especificações pé direito com elementos do protocolo","observacoes_esquerdo":"especificações pé esquerdo com elementos do protocolo"},"exercicios":["exercício 1 do protocolo","exercício 2","exercício 3","exercício 4"],"retorno":"prazo","urgencia":"eletivo"}`;
  ...
  async function enviarAudioParaTranscrever(){
    ...
    const permIA = podeUsarIAqtd(clinicaId, planoIA, 10); // transcrição de áudio consome 10
            ...
            <Card p={20} style={{border:`1px solid ${C.purple}30`,background:`${C.purple}06`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
                <div>
                  ...
                  <div style={{fontSize:11.5,color:C.muted,marginTop:3,lineHeight:1.5}}>Grava a conversa e transcreve com IA (Whisper). Ideal para trechos de até <strong>5 minutos</strong>. O texto aparece no campo acima. <strong style={{color:C.amber}}>⚠️ Cada transcrição consome 10 análises da sua cota de IA</strong>.</div>
                </div>
                {!gravandoAudio
                  ? <Btn v="primary" disabled={transcrevendoAudio} onClick={iniciarAudioConsulta} style={{background:C.purple}}>{transcrevendoAudio?<><Spin sz={14}/> Transcrevendo...</>:"🎧 Iniciar gravação (10 análises)"}</Btn>
...
function DashClinica({pedidos,pacientes,clinicaObj,hideValues,onNavegar,planoIA}) {
  ...
  return (
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      ...
      {iaEsgotando && (
        <Card hover={false} p={0} style={{border:`1px solid ${pctIA>=1?C.red:C.amber}45`,overflow:"hidden"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,padding:"13px 16px",background:`${pctIA>=1?C.red:C.amber}0C`,flexWrap:"wrap"}}>
            <div style={{display:"flex",alignItems:"center",gap:11}}>
              <span style={{fontSize:22}}>{pctIA>=1?"🚫":"⚡"}</span>
              <div>
                <div style={{fontSize:13,fontWeight:800,color:pctIA>=1?C.red:C.amber}}>{pctIA>=1?`Sua cota de IA do mês acabou (${usoIA}/${limiteIA})`:`Sua cota de IA está acabando: ${usoIA}/${limiteIA} análises usadas`}</div>
                <div style={{fontSize:11,color:C.muted,marginTop:2}}>Garanta <strong style={{color:C.green}}>+50 análises por R$ 49,90</strong> e não pare seus atendimentos com IA.</div>
              </div>
            </div>
            <Btn v="primary" sz="sm" onClick={()=>onNavegar&&onNavegar("ia")}>✦ Comprar +50 análises →</Btn>
          </div>
        </Card>
      )}
...
function AppCore() {
  ...
  const renderPage = () => {
    ...
    if(userType==="clinica" || userType==="colaborador") {
      ...
      if(page==="dashboard")  return <DashClinica pedidos={meusPedidos} pacientes={meusPacientes} clinicaObj={clinicaObj} hideValues={ehColab} onNavegar={(pg)=>setPage(pg)} planoIA={planoIAefetivo}/>;
