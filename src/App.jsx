  ...
  async function gerarImagem() {
    ...
    try {
      const res = await fetch("/api/imagem", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: imgPrompt + ". Estilo: ilustração profissional moderna para clínica de fisioterapia e palmilhas posturais, cores vibrantes e harmoniosas (azuis, verdes e tons claros), iluminação suave, alta qualidade, visual limpo e acolhedor, composição equilibrada, sem texto na imagem." }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error((data && data.error && data.error.message) || ("Erro " + res.status));
      let imagemUrl = null;
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          {PLANOS_INFO.map(p => { const atual = p.nome === plano; return (
            <Card key={p.nome} p={20} style={{ border: atual ? `2px solid ${p.cor}` : `1px solid ${C.border}`, position: "relative" }}>
              ...
              {atual && p.preco > 0 && <div style={{ marginTop: 14 }}>
