  ...
  async function gerarImagem() {
    ...
    try {
      const res = await fetch("/api/imagem", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: imgPrompt + " — estilo profissional para clínica de fisioterapia e palmilhas posturais, alta qualidade, cores claras e saudáveis" }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error((data && data.error && data.error.message) || ("Erro " + res.status));
      if (data.b64) setImgResult("data:image/png;base64," + data.b64);
      else if (data.url) setImgResult(data.url);
      ...
      {aba === "ia" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <Card hover={false} p={20}>
            ...
            <div style={{ fontSize: 11, color: C.muted, marginBottom: 12 }}>Descreva a imagem para o seu post. A IA cria uma ilustração. <strong style={{color:C.amber}}>⚠️ Cada imagem gerada consome 30 análises da sua cota</strong> (imagem custa bem mais que texto).</div>
            ...
            {imgResult && (
              <div style={{ marginTop: 12 }}>
                ...
                <a href={imgResult} download="fisiopiede-marketing.png" style={{ textDecoration: "none" }}><Btn v="outline" sz="sm" full style={{ marginTop: 8, justifyContent: "center" }}>⬇️ Baixar imagem</Btn></a>
              </div>
            )}
          </Card>
        </div>
